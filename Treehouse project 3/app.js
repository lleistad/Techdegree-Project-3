...
$("#color option").each(function(index){
  const $noColorOption = $("#color option[value='select a theme']");
  const $themeOption = $("#design option:selected");
  ...
  //Show only "Please Select a Tshirt Theme" in color dropdown menu when no theme is chosen
  if ($themeOption === 'select_theme') {
    $noColorOption.show();
  } else if ($themeOption === 'js puns') {
    //Show only "Cornflower Blue," "Dark Slate Grey," and "Gold" in color dropdown menu
    $cornflowerBlue.show();
    $darkslateGrey.show();
    $gold.show();
  } else if ($themeOption === 'heart js') {
    //Show only "Tomato," "Steel Blue," and "Dim Grey" in color dropdown menu
    $tomato.show();
    $steelblue.show();
    $dimgrey.show();
  }
